<template>
    <view>
        <view class="list-cell" hover-class="uni-list-cell-hover" @click="bindClick(data)">
            <view class="media-list" v-if="data.name">
                <view :class="[isImgRight?'media-image-right':'',isImgLeft?'media-image-left':'']">
                    <rich-text :class="['media-title',isImgRight||isImgLeft?'media-title2':'']" :nodes="data.name"></rich-text>
                ...
                <view class="media-foot">
                    <view class="media-info">
                        ...
                        <text class="info-text">内容:{{data.threads}}</text>
                        <text class="info-text">{{timestr(data.btime)}}</text>
                    </view>
                   
					<view class="max-close-view">
					<uni-icon size="30" type="star-filled" :class="data.follow?'red':''"></uni-icon>
		</view>
					                    </view>
...
<script>
	...
	import mask from '@/static/ysv8/f.js';
